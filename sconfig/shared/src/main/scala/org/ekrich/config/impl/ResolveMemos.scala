...
final class ResolveMemos private ( // note that we can resolve things to undefined (represented as Java null,
    ...
    val memos: BadMap[MemoKey, AbstractConfigValue | Null]
) {
  def this() = this(new BadMap[MemoKey, AbstractConfigValue | Null])

  private[impl] def get(key: MemoKey): AbstractConfigValue | Null = memos.get(key)

  private[impl] def put(key: MemoKey, value: AbstractConfigValue | Null) =
