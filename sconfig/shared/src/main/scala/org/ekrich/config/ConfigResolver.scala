...
trait ConfigResolver {
   ...
   * [[ConfigUtil$.splitPath ConfigUtil.splitPath(String)]]. If a non-null value
   * is returned that value will be substituted, otherwise resolution will
   * continue to consider the substitution as still unresolved.
   *
   * @param path
   *   the unresolved path
   * @return
   ...
   * doesn't provide a substitution itself.
   *
