...
trait ConfigIncluder {
   ...
   * specify, then the same class implementing {@link ConfigIncluder} must also
   * implement {@link ConfigIncluderClasspath}, {@link ConfigIncluderFile}, or
   * {@link ConfigIncluderURL} as needed, or a default includer will be used.
   *
   * @param context
